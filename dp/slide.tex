...
\begin{frame}{状压 DP：优化技巧}
    ...
    \begin{block}{meet-in-the-middle 技巧}
...
\begin{frame}{概率与期望 DP：状态定义}
    \begin{alertblock}{经典困惑}
    为什么设计 DP 状态必须是"当前状态到达终止状态"的概率或期望，而不能是"从初始状态到当前状态"？
    \end{alertblock}
    
    \begin{block}{根本原因}
    \begin{itemize}
        \item 没有澄清 DP 的对象
        \item 普通计数 DP 可以设计"从初始状态到当前状态"
        \item 但期望 DP 会有歧义：条件期望 vs 期望和
        \item 转移是要乘概率的，导致转移意义与状态意义不符
    \end{itemize}
    \end{block}
    
    \begin{block}{推荐方案}
    设计成"当前状态到达终止状态"可以避免这些问题，从而比较简洁地直接转移。
    \end{block}
\end{frame}

\begin{frame}{概率与期望 DP：技巧}
    \begin{block}{计数转概率/期望}
    某些计数问题可以通过转成求概率或期望来简化转移公式：
    \begin{itemize}
        \item 直接 DP 需要记方案数和总和
        \item 化成期望 DP，方案数直接变成恒为 $1$ 的概率
        \item 可以忽略结构中对状态没有影响的元素
    \end{itemize}
    \end{block}
    
    \begin{block}{期望的拆分}
    $$E[\text{总数}]=E[\text{轮数}]\cdot E[\text{单轮数}]$$
    当满足独立性时，可以分别计算后相乘。
    \end{block}
\end{frame}
...
\begin{frame}{区间 DP}
    这部分不是指区间 DP，而是某些给定若干区间然后选的问题。
    
    \begin{block}{初步处理方式}
    \begin{enumerate}
        \item 按某个端点排序
        \item 处理掉一些区间使得左右端点同时递增
        \item 或只有包含和相离情况
    \end{enumerate}
    \end{block}
    
    \begin{alertblock}{特点}
    这类问题较罕见，一个合理的状态能给出极好的无关性。
    \end{alertblock}
\end{frame}

\begin{frame}{数学方法在 DP 中的应用}
    \begin{block}{何时推式子 vs 何时 DP}
    \begin{itemize}
        \item 推式子往往需要更透彻的对模型的分析
        \item 可以利用各种数学工具得到不依赖于组合意义的、更本质的解法
        \item 但有时 DP 的复杂度反而更优
        \item 两者的分界并不明确
    \end{itemize}
    \end{block}
    
    \begin{block}{可以机械化的优化}
    \begin{enumerate}
        \item 矩乘相关：行列式、特征多项式、对角化等
        \item 常系数线性递推
        \item 整式递推
    \end{enumerate}
    \end{block}
...
\begin{frame}{通用解题方法：四个步骤}
    思考一道 DP 题的过程可以归纳为如下四步：
    
    \begin{enumerate}
        \item \textbf{分析模型性质}，简化模型，揭开模型"真面目"
        \item \textbf{处理模型}，找到可以用 DP 解决的对象
        \item \textbf{明确转化后的问题}，设计 DP 方式
        \item \textbf{优化 DP}
    \end{enumerate}
    ...
    \begin{itemize}
        \item 相邻两步之间没有明确的分界
        \item 思考过程并不一定是依次做这四个步骤
        \item 可能在第四步时会返回到前三步，反复寻找更优的 DP 方式
    \end{itemize}
    \end{alertblock}
\end{frame}

\begin{frame}{第一步：分析性质}
    \begin{block}{主要思考内容}
    \begin{enumerate}
        \item 寻找可以直接从题目条件导出，或与题目条件等价的条件
        \item 模拟（包括打表）小样例、特殊样例找到简化的方向和规律
        \item 想象某个最终解，利用（广义的）调整法排除可能性
        \item 联想一些与题目条件相关或类似的模型，尝试套用
    \end{enumerate}
    \end{block}
    
    \begin{block}{目标}
    减少总体（静态）的模型可能性，基于题目条件作有充分性的推导。
    \end{block}
\end{frame}

\begin{frame}{第二步：处理模型}
    通过改变（增删、合并、拆分、等价转述）\textbf{要素}：
    \begin{itemize}
        \item 调整这些要素的主次关系
        \item 调整自变量与因变量关系
        \item 调整组织顺序
        \item 调整枚举顺序
    \end{itemize}
    得到一个新的看待模型的方式，在新的模型描述下，所有解应与原模型的所有解形成一个映射（双射）。
    从信息量（不确定性）的角度考虑转化是否有用。
\end{frame}

\begin{frame}{第三步：设计 DP - 基本原则}
    \begin{block}{三个原则}
    \begin{enumerate}
        \item 每个希望被考虑的解均被恰当地计入，不希望被考虑的解均不被计入
        \item 状态数足够少
        \item 状态是封闭的（重叠子问题）
    \end{enumerate}
    \end{block}
    ...
    \begin{block}{三个意识}
    \begin{enumerate}
        \item 优先考虑优化暴力 DP 和推广特殊情况 DP
        \item 优先考虑组合意义，优化时结合组合意义
        \item 优先寻找突破口（贪心性质、结构简单特征、信息量小或限制多的子结构、无关性或独立性）
    \end{enumerate}
    \end{block}
\end{frame}

\begin{frame}{第三步：设计 DP - 设计方法}
    \begin{block}{方法一：设计生成过程，再确定状态}
    \begin{itemize}
        \item 首先确定一个解大致的生成顺序
        \item 然后确定生成过程中的部分结构或时刻
        \item 用状态去表出这部分，用转移表出状态之间的部分
    \end{itemize}
    核心和难点：生成顺序的选取和子结构的选取
    \end{block}
    
    \begin{block}{方法二：关注单步决策，推理出状态}
    \begin{itemize}
        \item 先通过观察生成过程中某个静态的片刻或局部
        \item 发现无关性，作为定状态的突破口
        \item 思考该部分与剩余待决策部分的关系
        \item 反复进行上述操作，不断扩充状态使其关于转移封闭
    \end{itemize}
    \end{block}
\end{frame}

\begin{frame}{第三步：设计 DP - 单步 vs 断点}
    \begin{block}{单步转移}
    \begin{itemize}
        \item 每次只决策一个"最小单元"
        \item 易写转移方程，不容易漏性质
    ...
    \begin{itemize}
        \item 一次性决策连续的待定部分
        \item 利用数学方法或其他性质
        \item 往往用于缩状态，但转移需要优化
    \end{itemize}
    \end{block}
    
    \begin{alertblock}{推荐}
    在数据范围允许的情况下，一般建议单步转移。
    \end{alertblock}
\end{frame}

\begin{frame}{第四步：优化 DP}
    \begin{alertblock}{重要提醒}
    最好的优化 DP 的方法，是做好前三步，或在较劣的 DP 基础上重做前三步。
    \end{alertblock}
    ...
    \begin{block}{优化分类}
    \begin{itemize}
        \item 对状态的优化：换元、合并、去除
        \item 对转移的优化：决策单调性、斜率优化、WQS 二分、数据结构
    \end{itemize}
    \end{block}
\end{frame}
